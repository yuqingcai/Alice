...
class SquareModel: ComposeModel {
    ...
    override func reset(hue: Int32, saturation: Int32, brightness: Int32) -> ColorScheme {
        ...
        let hues: Array<Int32> = [
            ...
            ColorFunction.hueRotate(from: hue, with: hueDistance*3, in: .antiClockwise),
        ]
        
        // 5 saturations
        var saturations: Array<Int32> = Array(repeating: 0, count: hues.count)
        saturations[0] = difference(to: saturation, with: -30 , in: 0...100)
        saturations[1] = difference(to: saturation, with: -10 , in: 0...100)
        saturations[2] = difference(to: saturation, with:  0  ,  in: 0...100)
        saturations[3] = difference(to: saturation, with: -10 , in: 0...100)
        saturations[4] = difference(to: saturation, with: -15 , in: 0...100)
        
        // 5 brightnesses
        var brightnesses: Array<Int32> = Array(repeating: 0, count: hues.count)
        brightnesses[0] = difference(to: brightness, with:  0 , in: 0...100)
        brightnesses[1] = difference(to: brightness, with:  0 , in: 0...100)
        brightnesses[2] = difference(to: brightness, with:  0 , in: 0...100)
        brightnesses[3] = difference(to: brightness, with:  0 , in: 0...100)
    ...
    override func createHues(_ selector: Int, _ hue: Int32?) -> Array<Int32> {
        ...
        if (selector == 0 || selector == 2) {
            ...
            hues[3] = ColorFunction.hueRotate(from: hue, with: hueDistance*2, in: .antiClockwise)
            hues[4] = ColorFunction.hueRotate(from: hue, with: hueDistance*3, in: .antiClockwise)
        }
        else if (selector == 1) {
            hues[1] = hue
            hues[0] = ColorFunction.hueRotate(from: hue, with: hueDistance, in: .clockwise)
            hues[2] = ColorFunction.hueRotate(from: hue, with: hueDistance, in: .clockwise)
            hues[3] = ColorFunction.hueRotate(from: hue, with: hueDistance, in: .antiClockwise)
            hues[4] = ColorFunction.hueRotate(from: hue, with: hueDistance*2, in: .antiClockwise)
        }
        else if (selector == 3) {
            hues[3] = hue
            hues[1] = ColorFunction.hueRotate(from: hue, with: hueDistance, in: .clockwise)
            hues[0] = ColorFunction.hueRotate(from: hue, with: hueDistance*2, in: .clockwise)
            hues[2] = ColorFunction.hueRotate(from: hue, with: hueDistance*2, in: .clockwise)
            ...
            hues[1] = ColorFunction.hueRotate(from: hue, with: hueDistance*2, in: .clockwise)
            hues[0] = ColorFunction.hueRotate(from: hue, with: hueDistance*3, in: .clockwise)
            hues[2] = ColorFunction.hueRotate(from: hue, with: hueDistance*3, in: .clockwise)
        }
