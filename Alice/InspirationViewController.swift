...
class InspirationViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func setupButtons() {
        ...
        let items = [
            UIAction(title: NSLocalizedString("Subscription-MenuItem", comment: ""), image: UIImage(named: "icon-coffee"), handler: { (_) in
    ...
    func configureNavigationbar() {
        ...
        navigationBar.topItem?.title = NSLocalizedString("Inspiration-NavigationTitle", comment: "")
    }
    
    @IBAction func networkDisabledNoticationResponder (_ notification:NSNotification) {
        DispatchQueue.main.async {
            self.performSegue(withIdentifier: "IDSegueInspirationToNetworkNotification", sender: self)
        }
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "IDSegueInspirationToNetworkNotification" && segue.destination is NetworkNotificationViewController) {
