...
int Mark(struct box *cube, int label, unsigned char *tag)
{
    register int r, g, b;
         ...
         int colorCount)
{
    ...
    next = 0;
        for(i=1; i<K; ++i){
            ...
            if (temp <= 0.0) {
              K = i+1;
              fprintf(stderr, "Only got %ld boxes\n", K);
              break;
        }
    }
    ...
    unsigned int* pixels = (unsigned int*)malloc(sizeof(unsigned int)*size);
    for (int y = 0; y < height; y ++) {
        for (int x = 0; x < width; x ++) {
            unsigned int i = y*width+x;
            pixels[i] = lut_r[Qadd[i]] << 24 |
                lut_g[Qadd[i]] << 16 |
                lut_b[Qadd[i]] << 8 |
                0xFF;
        }
